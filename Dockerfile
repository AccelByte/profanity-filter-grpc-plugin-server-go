...
RUN go build -v -modcacherw -o /output/$TARGETOS/$TARGETARCH/profanity-filter-grpc-plugin-server-go .
...
# Copy build
COPY --from=builder /output/$TARGETOS/$TARGETARCH/profanity-filter-grpc-plugin-server-go profanity-filter-grpc-plugin-server-go
